  ...
  return (
    <div className="relative">
      <Input
        ...
        className={className || "w-full !min-h-0 h-10 sm:h-9 text-sm"}
      />
      
      {open && (
        <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-popover text-popover-foreground border border-border rounded-xl shadow-lg max-h-64 overflow-auto divide-y divide-border/60">
          {loading && (
            ...
            <div>
              {items.map((item) => (
                <div
                  ...
                  className="p-3 min-h-[44px] flex flex-col justify-center hover:bg-muted/70 active:bg-muted cursor-pointer transition-colors"
                >
                  <div className="flex flex-col">
                    <span className="font-semibold text-xs sm:text-sm text-foreground">{item["Item Description"]}</span>
