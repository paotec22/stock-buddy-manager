...
export function OnboardingTour() {
  ...
  const checkTourStatus = useCallback(async () => {
    ...
    try {
      ...
      const shouldRun = !localCompleted && (!profile?.tour_completed || (profile?.tour_version || 0) < TOUR_VERSION);
...
function TooltipComponent({ children, ...props }: { children: React.ReactNode; [key: string]: unknown }) {
  return (
    <div {...props} className="relative">
      <div className="absolute -top-2 -right-2 flex items-center justify-center w-6 h-6 rounded-full bg-muted/80 backdrop-blur opacity-0 group-hover:opacity-100 transition-opacity">
...
export function OnboardingTrigger() {
  ...
  const handleRestart = async () => {
    ...
    setShowRestart(true);
    setTimeout(() => setShowRestart(false), 100);
  };
