...
-- Enhance customer data protection - add audit logging for invoice access
...
-- Create function to log invoice access
CREATE OR REPLACE FUNCTION public.log_invoice_access()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.access_logs (user_id, table_name, action_type, record_id)
  VALUES (auth.uid(), TG_TABLE_NAME, TG_OP, NEW.id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

-- Add audit trigger for invoice access
DROP TRIGGER IF EXISTS log_invoice_access_trigger ON public.invoices;
CREATE TRIGGER log_invoice_access_trigger
  AFTER SELECT ON public.invoices
  FOR EACH ROW
  EXECUTE FUNCTION public.log_invoice_access();
